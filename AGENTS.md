...
GitHub Actions). This file is the tool-agnostic conventions guide, read by Codex, Copilot, Cursor and
others. Claude Code loads it via an `@AGENTS.md` import in the repository's local `CLAUDE.md`.
...
> `${SECRET_INTERNAL_DOMAIN}` placeholders, which Flux substitutes from `cluster-secrets` at
> apply-time. Address and lookup tables that need real device addresses must be templated inside an
...
- **HelmRelease `spec` order** is `interval` → `chartRef` → `dependsOn` → `values`. This repository
  orders `interval` before `chartRef`; most HRs omit `install`/`upgrade` and inherit them from the
  root Kustomization.
...
- **App names avoid hyphens so the host stays clean.** The route host follows
  ...
  `reactiveresume`, not `reactive-resume`. Keep the standard `{{ .Release.Name }}` host instead
...
`secretStoreRef.kind: ClusterSecretStore`, `name: onepassword-connect`, which reads the `Talos`
...
- `ConfigMap` resources must set `metadata.namespace` explicitly (Checkov CKV_K8S_21 fails the
  `default` namespace).
- Flux `postBuild` replaces `${VAR}` against `cluster-secrets`, and **undefined vars become empty
...
- **Anchored ports are unquoted integers** (`PORT: &port 3000`). Kubernetes rejects a quoted `"3000"`
  reused for a probe `httpGet.port` at apply time ("must contain at least one letter").
...
comment. There is no GitHub Actions render workflow. GitHub Actions still run security
...
Codex code review reads this section, as does any other reviewer that honours this file. A
Renovate-facing reviewer already runs here: the `claude/renovate-review` commit status is the **gate**
on dependency PRs (upstream changelog research, breaking-change verdict, required check on `main`).
Reviews driven by this section are **advisory**, so spend them on how the diff wires into _this_
repository rather than on re-deriving upstream release notes.
...
- **A literal `${VAR}` left unescaped.** Flux `postBuild` substitutes `${VAR}` against
  ...
  template, or shell snippet that needs the literal must escape it as `$${VAR}`. This fails
  silently: the config deploys blank instead of erroring.
...
- A `ConfigMap` with no explicit `metadata.namespace`. Checkov CKV_K8S_21 fails the `default`
  namespace.
- A quoted anchored port (`PORT: &port "3000"`) reused as a probe `httpGet.port`. Kubernetes rejects
  it at apply time with "must contain at least one letter"; ports are unquoted integers.
...
- A bump to cluster-critical infrastructure that needs a repo-side change the PR does not make: a
  renamed Helm value, a CRD version the manifests still pin, a required migration step. The
...
- YAML anchors (`&app`, `&namespace`, `*app`) in `ks.yaml`. They are the house DRY pattern, not
  duplication.
