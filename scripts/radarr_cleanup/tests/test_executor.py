...
from radarr_cleanup.clients import SourceError
from radarr_cleanup.executor import (
    execute,
    recycle_bin_ready,
    reconcile,
    revalidate,
...
from radarr_cleanup.ledger import Ledger
from radarr_cleanup.model import Outcome
...
from radarr_cleanup.model import Assessment
...
class RecommendationValidationTests(unittest.TestCase):
    ...
    def test_unknown_movie_id_is_rejected(self):
        _, rejected = validate_recommendations(
            [{"movie_id": 99, "verdict": "delete", "reason": "looks like filler to me"}], self.candidates
        )
        self.assertIn("not in this run's candidate set", rejected[0]["why"])

    def test_bad_verdict_is_rejected(self):
        _, rejected = validate_recommendations(
            [{"movie_id": 1, "verdict": "obliterate", "reason": "because"}], self.candidates
        )
    ...
    def test_non_integer_id_is_rejected(self):
        _, rejected = validate_recommendations(
            [{"movie_id": "1", "verdict": "delete", "reason": "a perfectly good reason"}], self.candidates
        )
...
class RecycleBinTests(unittest.TestCase):
    ...
    def test_configured_bin_is_ready(self):
        ok, detail = recycle_bin_ready({"recycleBin": "/media/trash", "recycleBinCleanupDays": 14})
        self.assertTrue(ok)
    ...
    def test_zero_retention_blocks(self):
        ok, _ = recycle_bin_ready({"recycleBin": "/media/trash", "recycleBinCleanupDays": 0})
        self.assertFalse(ok)
...
class RevalidationTests(unittest.TestCase):
    ...
    def test_file_disappeared_blocks(self):
        ...
        self.assertFalse(revalidate(1, radarr, TAGS, set(), set(), {5000})[0])
...
class ExecutionTests(unittest.TestCase):
    ...
    def test_dry_run_writes_intent_but_never_an_outcome(self):
        ...
        self.assertFalse(any("/outcome/" in k for k in keys), "dry run must record no completed actions")
    ...
    def test_http_error_is_a_failure_not_a_deletion(self):
        ...
        result = execute([assessment()], radarr, FakeTautulli(), self.ledger(False), TAGS, set(), dry_run=False)
    ...
    def test_unavailable_activity_check_skips_rather_than_proceeds(self):
        ...
        result = execute([assessment()], radarr, tautulli, self.ledger(False), TAGS, set(), dry_run=False)
        self.assertEqual(radarr.deleted, [])
    ...
    def test_partial_failure_does_not_stop_the_batch(self):
        ...
        radarr.delete_behaviour[1] = "http-500"
        result = execute(
            [assessment(1), assessment(2)], radarr, FakeTautulli(), self.ledger(False), TAGS, set(), dry_run=False
        )
        self.assertEqual(len(result.failed), 1)
        self.assertEqual(len(result.deleted), 1)
