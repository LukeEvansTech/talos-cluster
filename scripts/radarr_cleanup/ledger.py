...
The cloud checkout is rebuilt on every run and the chat transcript is not a
record, so anything that must outlive a run lives in object storage. Two things
in particular have to be durable or the routine is unsafe: what it *intended* to
do before it did it, and what a person has already decided.
...
class Ledger:
    ...
    def write_first_seen(self, index: dict[int, datetime]) -> None:
        ...
        Write-once per film: an upgrade or a rename must never push the clock
        forward, or a film can sit permanently just inside its own window.
        """
    ...
    def write_crosswalk(self, crosswalk: dict[int, dict]) -> None:
        ...
        Plex reissues a rating key when an item is replaced, and the old key is
        still what the old history rows carry, so dropping it loses those plays.
        """
    ...
    def record_outcome(self, movie_id: int, status: str, detail: str) -> None:
        """Write what actually happened, including "uncertain".

        A dry run records nothing here. A rehearsal that leaves completed-action
        records behind would make the next run believe the work was already done.
        """
        if self.dry_run:
            return
        self._write(
            f"runs/{self.run_id}/outcome/{movie_id}.json",
            {
                "run_id": self.run_id,
                "movie_id": movie_id,
                "status": status,
    ...
    def deletions_in_window(self, days: int = 7) -> int:
        ...
        The per-run cap is only a real limit if a second run starting while the
        first is still going counts what the first already did.
        """
    ...
    def unreconciled_intents(self) -> list[dict]:
        ...
        These are the interrupted deletions -- a timeout, a killed run -- and
        they must be reconciled against live Radarr before anything new happens.
        """
    ...
    def acquire_lock(self) -> tuple[bool, str]:
        ...
        Object storage gives no compare-and-set here, so this is a read-then-write
        and a determined race can still pass it. It is a guard, not a mutex: the
        thing that actually bounds concurrent damage is ``deletions_in_window``,
        which counts what other runs already did.
        """
        ...
        if existing:
            ...
            if expires > now and existing.get("run_id") != self.run_id:
                return False, f"run {existing.get('run_id')} holds the lock until {expires.isoformat()}"
        self._write(
            LOCK_KEY,
            {
                "run_id": self.run_id,
