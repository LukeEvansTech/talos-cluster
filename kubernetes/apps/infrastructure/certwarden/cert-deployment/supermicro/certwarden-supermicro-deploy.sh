#!/bin/bash
#
# Certwarden Post-Process Script for Supermicro IPMI Certificate Deployment
#
...
echo "=== Certwarden Supermicro Certificate Deployment ==="
        ...
        - name: supermicro-deploy
          ...
          command:
            ...
            - |
              ...
              python3 /scripts/supermicro-updater.py \\
                --ipmi-url "\${IPMI_URL}" \\
                --model "\${IPMI_MODEL}" \\
...
if [[ "${JOB_STATUS}" == "True" ]]; then
    echo "✅ Certificate deployed successfully to ${SUPERMICRO_HOST}"
    exit 0
else
    echo "❌ Failed to deploy certificate to ${SUPERMICRO_HOST}"
    kubectl logs "job/${JOB_NAME}" -n "${NAMESPACE}"
    exit 1
fi
