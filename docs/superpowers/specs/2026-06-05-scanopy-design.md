...
Current upstream release: **v0.16.2** (2026-04-28) — also the version the reference repository pins.
...
| SNMP              | **In** — community via ExternalSecret, mounted read-only at neutral path | Community string lives only in 1Password (repository is PUBLIC). Mount path `/run/secrets/snmp-community` — no vendor name in Git. SNMP credential then configured in scanopy UI pointing at the file.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
Convention notes verified against the repository:
...
> onepassword dependency name in this repository is `onepassword` in `external-secrets` (confirm).
...
Repository is PUBLIC. No LAN IPs, no `.lan`/`.internal` hostnames, no device/vendor names in Git.
