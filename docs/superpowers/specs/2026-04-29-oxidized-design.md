...
Deploy [Oxidized](https://github.com/ytti/oxidized) — a network device configuration backup tool — into the `observability` namespace, polling six homelab network devices daily and pushing config diffs to a private GitHub repository (`LukeEvansTech/network-configs`). Notifications go to Pushover (drift via Oxidized's `exec` hook; stale-device alerts via the existing kube-prometheus-stack → Alertmanager → Pushover route).

Purpose: drift detection. The Terraform/Ansible IaC in the sibling `network-ops` repository describes _desired_ state; Oxidized snapshots _actual_ running state, catching out-of-band changes (manual GUI/SSH edits) that bypass IaC.
...
| Ruckus AP/controller        | `ruckus.lan`          | `ruckusunleashed` _(model TBC during pre-deploy)_ |
...
Single-pod deployment in `observability`, sitting next to `snmp-exporter`. Oxidized polls devices on a 24h interval, writes to a local bare Git repository on a Ceph PVC, and pushes commits to GitHub via a deploy key. A sidecar Prometheus exporter scrapes Oxidized's `/nodes.json` and exposes per-device last-success metrics.
...
- `postBuild.substitute`:
    ...
    - `GATUS_SUBDOMAIN: oxidized` _(if you want a Gatus check — optional)_
            ...
            router-db-render:
                ...
                env:
                    ...
                    ONYX_USERNAME:
                        {
                            secretKeyRef:
                                { name: oxidized-secret, key: ONYX_USERNAME },
                        }
                    ONYX_PASSWORD:
                        {
                            secretKeyRef:
                                { name: oxidized-secret, key: ONYX_PASSWORD },
                        }
                    ...
                    RUCKUS_USERNAME:
                        {
                            secretKeyRef:
                                { name: oxidized-secret, key: RUCKUS_USERNAME },
                        }
                    RUCKUS_PASSWORD:
                        {
                            secretKeyRef:
                                { name: oxidized-secret, key: RUCKUS_PASSWORD },
                        }
        containers:
            app:
                image:
...
| GitHub repository `LukeEvansTech/network-configs` exists, private                          | `gh repo view LukeEvansTech/network-configs`                                        |
| Deploy key with **write** access added to repository                                       | `gh repo deploy-key list --repo LukeEvansTech/network-configs`                      |
...
- GitHub repository private. Configs may contain topology/IP info even after `remove_secret` filters.
...
2. Confirm GitHub repository + deploy key.
...
5. Confirm GitHub repository shows 6 config files.
...
- **OPNsense secret stripping** — verify after first poll whether the committed `config.xml` contains plaintext secrets; add `remove_secret` filter and any custom regular expression if so.
