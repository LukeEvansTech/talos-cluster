...
**Testing model:** This repository has no unit tests; "test" steps are local-validation (`yamlfmt`, `flux-local build ks`, `talhelper validate`) plus post-merge cluster gates (`kubectl`, `talosctl`, functional smoke tests).
...
> **PRE-REQUIREMENT:** All four PR2 verification gates passed. Pick a low-traffic window (no active Plex streams, no Tdarr jobs, no Ollama generations, no Whisper inference).

> **POST-PR2 PATH/SCOPE CORRECTIONS (applied 2026-04-29 after merging PR2):**
>
> - PR2 shipped at `kubernetes/apps/gpu-operator/gpu-operator/` in its own `gpu-operator` namespace, NOT in `kube-system`. All PR3 paths below reflect this.
> - The operator Kustomization is named `gpu-operator` in namespace `gpu-operator`. Workload `dependsOn` targets that, not `kube-system`.
> - The actual workload count is **8**, not 4: `ai/{ollama,openaiwhisper,whisper}` and `media/{dispatcharr,jellyfin,pinchflat,plex,tdarr}`. Selector flips touch all 8.
> - Pinchflat's `ks.yaml` has no GPU `dependsOn` today (only `rook-ceph-cluster`); we add `gpu-operator` while we're in there.
> - PR4 adds `media/dispatcharr` to env-var cleanup (lines 37–38).

### Task 3.1: Update HelmRelease to enable devicePlugin and dcgmExporter

**Files:**

- Modify: `kubernetes/apps/gpu-operator/gpu-operator/app/helmrelease.yaml`
...
- Create: `kubernetes/apps/gpu-operator/gpu-operator/app/runtimeclass.yaml` (via `git mv`)
- Modify: `kubernetes/apps/gpu-operator/gpu-operator/app/kustomization.yaml`
...
git mv kubernetes/apps/kube-system/nvidia-device-plugin/app/app/runtimeclass.yaml \
       kubernetes/apps/gpu-operator/gpu-operator/app/runtimeclass.yaml
```
...
Edit `kubernetes/apps/gpu-operator/gpu-operator/app/kustomization.yaml`:
...
- Move: `kubernetes/apps/observability/dcgm-exporter/app/grafanadashboard.yaml` → `kubernetes/apps/gpu-operator/gpu-operator/app/grafanadashboard.yaml`
- Move: `kubernetes/apps/observability/dcgm-exporter/app/prometheusrule.yaml` → `kubernetes/apps/gpu-operator/gpu-operator/app/prometheusrule.yaml`

- [ ] **Step 1: Move both files**

```bash
git mv kubernetes/apps/observability/dcgm-exporter/app/grafanadashboard.yaml \
       kubernetes/apps/gpu-operator/gpu-operator/app/grafanadashboard.yaml
git mv kubernetes/apps/observability/dcgm-exporter/app/prometheusrule.yaml \
       kubernetes/apps/gpu-operator/gpu-operator/app/prometheusrule.yaml
```
...
Edit `kubernetes/apps/kube-system/kustomization.yaml`. Remove the `./nvidia-device-plugin/ks.yaml` line. (Note: `gpu-operator` is NOT in `kube-system` — it lives at `kubernetes/apps/gpu-operator/`, so don't add it here.)
...
### Task 3.6: Swap workload affinity selectors (8 files)

**Files:**

- Modify: `kubernetes/apps/ai/ollama/app/helmrelease.yaml`
- Modify: `kubernetes/apps/ai/openaiwhisper/app/helmrelease.yaml`
- Modify: `kubernetes/apps/ai/whisper/app/helmrelease.yaml`
- Modify: `kubernetes/apps/media/dispatcharr/app/helmrelease.yaml`
- Modify: `kubernetes/apps/media/jellyfin/app/helmrelease.yaml`
- Modify: `kubernetes/apps/media/pinchflat/app/helmrelease.yaml`
...
Audited line ranges (2026-04-29) per workload:

- [ ] **Step 1: Edit ollama** — `kubernetes/apps/ai/ollama/app/helmrelease.yaml` (matchExpressions at 47–51)
- [ ] **Step 2: Edit openaiwhisper** — `kubernetes/apps/ai/openaiwhisper/app/helmrelease.yaml` (57–61)
- [ ] **Step 3: Edit whisper** — `kubernetes/apps/ai/whisper/app/helmrelease.yaml` (43–47)
- [ ] **Step 4: Edit dispatcharr** — `kubernetes/apps/media/dispatcharr/app/helmrelease.yaml` (60–64)
- [ ] **Step 5: Edit jellyfin** — `kubernetes/apps/media/jellyfin/app/helmrelease.yaml` (64–68)
- [ ] **Step 6: Edit pinchflat** — `kubernetes/apps/media/pinchflat/app/helmrelease.yaml` (58–62)
- [ ] **Step 7: Edit plex** — `kubernetes/apps/media/plex/app/helmrelease.yaml` (99–103)
- [ ] **Step 8: Edit tdarr** — `kubernetes/apps/media/tdarr/app/helmrelease.yaml` (75–79)

### Task 3.7: Swap dependsOn in workload Kustomizations (7 files + 1 add)

**Files (flip nvidia-device-plugin → gpu-operator):**

- Modify: `kubernetes/apps/ai/ollama/ks.yaml`
- Modify: `kubernetes/apps/ai/openaiwhisper/ks.yaml`
- Modify: `kubernetes/apps/ai/whisper/ks.yaml`
- Modify: `kubernetes/apps/media/dispatcharr/ks.yaml`
...
**Files (add new dependsOn entry — pre-existing gap):**

- Modify: `kubernetes/apps/media/pinchflat/ks.yaml`

In each of the 7 files above, find:
...
> Note: target namespace is `gpu-operator`, not `kube-system`. Other entries in `dependsOn` (`volsync`, `rook-ceph-cluster`) stay unchanged.

For pinchflat, prepend a new entry to the existing `dependsOn:` (currently only has `rook-ceph-cluster`):
...
- [ ] **Step 1: Edit all 7 nvidia-device-plugin ks.yaml files** (flip).
- [ ] **Step 2: Edit pinchflat ks.yaml** (add).
...
```bash
yamlfmt kubernetes/apps/gpu-operator/gpu-operator/ \
        kubernetes/apps/{ai/ollama,ai/openaiwhisper,ai/whisper,media/dispatcharr,media/jellyfin,media/pinchflat,media/plex,media/tdarr}/{ks.yaml,app/helmrelease.yaml} \
...
```bash
for app in ollama openaiwhisper whisper dispatcharr jellyfin pinchflat plex tdarr; do
  echo "=== $app ==="
  flux-local build ks --path kubernetes/flux/cluster $app || break
done
```

Expected: all 8 build cleanly.
...
- 8 GPU workloads (ollama, openaiwhisper, whisper, dispatcharr, jellyfin, pinchflat, plex, tdarr) re-pointed at GFD-managed labels
- pinchflat ks.yaml gains a gpu-operator dependsOn entry (pre-existing gap)
- Standalone Helm releases removed; Grafana dashboard and PrometheusRule preserved alongside operator

## Test plan
- [ ] kubectl -n gpu-operator get ds | grep -E 'nvidia|dcgm' → operator's DaemonSets running, standalone gone
- [ ] kubectl get nodes -o json | jq '.items[].status.allocatable["nvidia.com/gpu"]' → "5" on each
- [ ] kubectl get pods -A -l gpu-workload=true → all 8 workloads Running
...
- 8 workload Kustomizations reconcile with new selector + dependsOn (ollama, openaiwhisper, whisper, dispatcharr, jellyfin, pinchflat, plex, tdarr)

- [ ] **Step 3: Gate 1 — DaemonSets**

```bash
kubectl -n gpu-operator get ds
kubectl -n kube-system get ds | grep -i nvidia || echo "(none — expected)"
kubectl -n observability get ds | grep -i dcgm || echo "(none — expected)"
```

Expected:

- In `gpu-operator`: operator-owned DaemonSets including `nvidia-device-plugin-daemonset` and `nvidia-dcgm-exporter`, `READY 3/3` each.
- In `kube-system`: standalone `nvidia-device-plugin` is gone.
- In `observability`: standalone `dcgm-exporter` is gone.
...
Expected: all 8 workloads `Running` and `Ready` (ollama, openaiwhisper, whisper, dispatcharr, jellyfin, pinchflat, plex, tdarr).
...
5. Whisper / openaiwhisper / dispatcharr / pinchflat: confirm pods stay Running with no restart loop (no targeted UI smoke test required — pod-level health is sufficient since these don't continuously stream GPU work).
...
### Task 4.3a: Dispatcharr (added 2026-04-29 after audit)

**Files:**

- Modify: `kubernetes/apps/media/dispatcharr/app/helmrelease.yaml`

- [ ] **Step 1: Remove env lines (currently lines 37-38)**
...
```bash
yamlfmt kubernetes/apps/media/{plex,jellyfin,tdarr,dispatcharr}/app/helmrelease.yaml
```

- [ ] **Step 2: Validate kustomizations build**

```bash
for app in plex jellyfin tdarr dispatcharr; do
  echo "=== $app ==="
  flux-local build ks --path kubernetes/flux/cluster $app || break
done
```
...
```bash
git add kubernetes/apps/media/{plex,jellyfin,tdarr,dispatcharr}/app/helmrelease.yaml
...
## Summary
- Removes NVIDIA_VISIBLE_DEVICES and NVIDIA_DRIVER_CAPABILITIES from plex, jellyfin, tdarr (both containers), and dispatcharr
- Ollama, openaiwhisper, whisper, pinchflat already don't set them
...
## Test plan
- [ ] After merge, kick each pod: kubectl rollout restart -n media deploy/plex deploy/jellyfin deploy/tdarr deploy/dispatcharr
- [ ] Plex hardware-transcodes a stream
- [ ] Jellyfin hardware-transcodes a stream
- [ ] Tdarr processes a job
- [ ] Dispatcharr stays healthy post-restart
EOF
)"
```
...
- Whether DCGM `ServiceMonitor` labels under the operator differ from the standalone (PrometheusRule references metric _names_, which are stable; if labels diverge, Task 3.10 Gate 5 surfaces it).
