...
PVC data is backed up with **VolSync** using the **Kopia** mover, to an NFS destination and a remote
target. Backups are opt-in per app via the `volsync` component.
...
- Schedules run nightly; snapshots should reach a Succeeded state.
